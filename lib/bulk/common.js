...
function mergeBatchResults(batch, bulkResult, err, result) {
  ...
  if (result.opTime || result.lastOp) {
    ...
    if (opTime._bsontype === 'Timestamp') {
      ...
      if (typeof opTime.ts === 'number') {
        opTime.ts = Timestamp.fromNumber(opTime.ts);
      }
      if (typeof opTime.t === 'number') {
        opTime.t = Long.fromNumber(opTime.t);
      }
    }
    ...
    } else if (opTime.ts.greaterThan(bulkResult.lastOp.ts)) {
      bulkResult.lastOp = opTime;
    } else if (
      opTime.ts.equals(bulkResult.lastOp.ts && opTime.t.greaterThan(bulkResult.lastOp.t))
    ) {
      bulkResult.lastOp = opTime;
    }
  }
